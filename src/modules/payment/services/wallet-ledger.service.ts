import { Injectable, Logger, InternalServerErrorException, NotFoundException, BadRequestException, HttpException, HttpStatus } from '@nestjs/common';
...
export class WalletLedgerService {
  ...
  async deductFare(
    ...
    const passenger = await this.passengerModel.findById(passengerId).session(session);
    if (!passenger) {
      throw new NotFoundException(`Passenger ${passengerId} not found`);
    }

    const currentBalance = parseFloat(passenger.walletBalance?.toString() || '0');
  ...
  async creditFromTopUp(
    ...
    try {
      ...
      if (existingTx) {
        this.logger.warn(`Payment Intent ${stripePaymentIntentId} already processed. Skipping.`);
      ...
      this.logger.log(`Credited LKR ${amount} to passenger ${passengerId}`);
    } catch (error) {
      await session.abortTransaction();
